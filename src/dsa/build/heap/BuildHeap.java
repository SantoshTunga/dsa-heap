...
public class BuildHeap {
	...
	{
		/** the given array is a non heap , so we need to construct the heap from the left side right most element 
		 * to get the left side right most element from the parent node is (n/2)-1 where n is lenght of the array
		 ***/
		int len = arr.length;
	...
	{
		/** Mean heap construct the element  from the root by comparing parent with its left or right child so the root always smaller compared to its 
